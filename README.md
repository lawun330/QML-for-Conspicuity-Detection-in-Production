...
4. We implement and analyze a quantum machine learning model for approximating the sine function. In `Task 4. QML_for_Sine_Function.ipynb`, we design a suitable quantum feature map, build and train a variational quantum circuit, and compare its regression predictions against the true sine function.

5. We prepare and model a weld defect dataset for conspicuity detection in a production-like setting. In `Task 5.1. QML_for_Conspicuity_Detection.ipynb`, we select a balanced subset of six defect classes, resize and normalize the images, and organize them into reproducible train/test splits in the `conspicuity_production_data` directory. In `Task 5.2. QML_for_Conspicuity_Detection.ipynb`, we build and compare a classical CNN and a quanvolutional neural network (QNN) on this dataset using a shared classifier architecture, evaluating them with validation curves, confusion matrices, and per-class metrics. Cached quanvolved data is saved in `quanvolution/conspicuity_production_data`.
